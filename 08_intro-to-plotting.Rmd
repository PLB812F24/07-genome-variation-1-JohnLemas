...
penguins = read.table("~/Documents/penguins.txt", header = T)
...
```{r}
ggplot(penguins, aes(x = penguins$flipper_length_mm)) + #information about the dataset
  geom_histogram(binwidth=3, color = "white", fill="navy") + #make the figure
...
ggplot(penguins, aes(x = body_mass_g, fill = species)) + 
  ...
  ylab('Frequency') + xlab('Body Mass (g)') +
  theme_classic()
```
...
ggplot(penguins, aes(x = bill_depth_mm, y = flipper_length_mm, color = species)) +
  geom_point() +
  theme_classic() +
  ylab('Flipper Length (mm)') + xlab('Bill Depth (mm)')
```
